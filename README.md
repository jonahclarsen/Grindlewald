...
Floodlights offer **Turn on if on WiFi: SSID**, **Turn on**, and **Turn off**. The Wi-Fi option captures the connected network when clicked, including when already selected, and keeps that SSID across page visits and app restarts. Selecting another option clears the saved network; its Wi-Fi label refreshes immediately and whenever you open Automations. At execution time, the Wi-Fi option turns on the floodlights only for an exact network match. An unavailable or different network leaves them unchanged. Grindlewald reads the connected network through CoreWLAN. On first opening Automations, allow its Location permission request: macOS requires that permission to reveal Wi-Fi names. The app does not request geographic coordinates. If access was denied, enable Grindlewald under **System Settings → Privacy & Security → Location Services**, then return to Automations. Permission and connection problems are shown separately; scheduled runs never prompt for permission. Existing “Do nothing” automations remain unchanged until you choose an option.

An automation may also run a shell command through `/bin/zsh -lc`. For unattended root commands, open **Settings** and install the **Privileged automations** service. macOS asks for administrator authorization once to install a root-owned copy of the Grindlewald helper and its narrowly scoped policy. Grindlewald never receives or stores the password. If the installed service needs an update or repair, a notice appears at the top of **Control**, with an action button to address it immediately.
...
The connection row above every page shows the current Bluetooth link status. Click the **Disconnect** button beside the connection status (also available while connecting) to release all light connections immediately and stop streamed effects without sending a power-off command. The status returns to **Disconnected** when the hold window expires. Using a light control again reconnects automatically; future scheduled automations still run.
